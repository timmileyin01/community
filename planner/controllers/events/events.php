...
if (isset($_POST['action']) && $_POST['action'] == "view") {
    ...
    $eventsView = "SELECT * FROM events WHERE user_id = ?";
    $eventsViewStmt = $conn->prepare($eventsView);
    $eventsViewStmt->bind_param("i", $user_id);
            ...
            if ($returnedEvent && $returnedEvent['event_id'] != $id && $returnedEvent['user_id'] == $user_id) {
                ...
                if (!empty($_FILES['image']['name'])) {
                    ...
                    if (count($errors) == 0) {
                        ...
                        if ($result) {
                            $sqlUpdate = "UPDATE events SET title = ?, description = ?, category_id = ?, date = ?, time = ?, center_id = ?, user_id = ?, flyer = ?, status = ? WHERE event_id = ?";
                            $stmtUpdate = $conn->prepare($sqlUpdate);
                            $stmtUpdate->bind_param("ssissiissi", $title, $description, $category_id, $date, $time,  $center_id, $user_id, $file_name, $status, $id);
                    ...
                    $sqlUpdate = "UPDATE events SET title = ?, description = ?, category_id = ?, date = ?, time = ?, center_id = ?, user_id = ?, status = ? WHERE event_id = ?";
                    $stmtUpdate = $conn->prepare($sqlUpdate);
                    $stmtUpdate->bind_param("ssissiisi", $title, $description, $category_id, $date, $time,  $center_id, $user_id, $status, $id);
                    $update = $stmtUpdate->execute();
...
if (isset($_POST['edit_id'])) {
    ...
    if ($viewEventResult) {
        $fullDetails = array("event_id"=>$id, "title" => $editEventResult['title'], "description" => $editEventResult['description'], "category_id" => $editEventResult['category_id'], "date" => $editEventResult['date'], "time" => $editEventResult['time'], "center_id" => $editEventResult['center_id'], "regular" => $viewEventResult['regular'], "vip" => $viewEventResult['vip'], "vvip" => $viewEventResult['vvip']);
    } else {
        $fullDetails = array("event_id"=>$id, "title" => $editEventResult['title'], "description" => $editEventResult['description'], "category_id" => $editEventResult['category_id'], "date" => $editEventResult['date'], "time" => $editEventResult['time'], "center_id" => $editEventResult['center_id']);
    }
